 ...
 */
export const Skeleton: React.FC<SkeletonCardProps> = ({
    width = '100%',
    height = '1rem',
    rounded = false,
    className = ''
    ...
    return (
        <div
            className={`skeleton-loading ${rounded ? 'rounded-full' : ''} ${className}`}
            style={{ width, height }}
            aria-hidden="true"
        />
    );
...
export const SkeletonCard: React.FC<SkeletonCardProps> = ({
    ...
    return (
        <div className={`bg-white rounded-xl border border-gray-200 overflow-hidden ${className}`}>
            {showImage && (
                <div className="skeleton-loading w-full" style={{ height: imageHeight }} />
            )}
