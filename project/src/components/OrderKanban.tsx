...
function OrderKanban() {
  ...
  const loadData = async () => {
    try {
      ...
      const [ordersResult, customersResult, teamMembersResult, teamsResult, leadsResult, quotesResult] = await Promise.all([
        ...
        getLeads(organisationId!, { status: 'new' }), // Fetch new leads
  ...
  const handleCreateQuote = async (lead: LeadWithRelations) => {
    ...
    try {
      setLoading(true);
      const result = await createQuoteFromLead(lead, organisationId!, lead.assigned_to_user_id);
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Säljtunnel</h1>
        ...
        <div className="bg-white shadow rounded-lg p-8">
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="text-sm text-gray-500">Laddar ordrar och leads...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              {orders.length} ordrar • {leads.length} förfrågningar • {quotes.length} offerter
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-7 gap-4">
        {kanbanColumns.map((column) => {
          ...
          return (
            <div
              key={column.status}
              className={`kanban-column rounded-lg border-2 border-dashed ${column.color} min-h-96`}
                    ...
                    <>
                      ...
                      {columnLeads.map((lead) => (
                        <div
                          ...
                          className="kanban-card bg-white rounded-lg p-4 shadow-sm border border-gray-200 group"
                        >
                          ...
                          <div>
                            ...
                            <div className="flex items-center mt-1">
                              {Array.from({ length: 5 }, (_, i) => (
                                <Star
                                  key={i}
                                  className={`w-4 h-4 ${i < selectedOrder.complexity_level! ? 'text-yellow-400 fill-current' : 'text-gray-300'
                                    }`}
                                />
                              ))}
