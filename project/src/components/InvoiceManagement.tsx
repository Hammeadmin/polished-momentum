...
import {
  getOrders, updateOrder as updateOrderInDb, type OrderWithRelations, getOrderNotes,
...
import { getCustomers, getTeamMembers, formatCurrency, formatDate, getSystemSettings, getSavedLineItems, getOrganisation, createSavedLineItem, checkDuplicateCustomer, createCustomer, searchCustomers } from '../lib/database';
...
function InvoiceManagement() {
  ...
  const loadData = async () => {
    try {
      ...
      if (!organisationId) {
        setError('Ingen organisation hittad.');
        setLoading(false);
        return;
      }
      const [
      ...
      if (templatesResult.error) {
        console.error("Could not fetch templates:", templatesResult.error);
      } else {
        // Filter to only show invoice templates
        const invoiceTemplates = (templatesResult.data || []).filter(t => t.settings?.template_type === 'invoice');
      ...
      const ordersResult = await getOrders(organisationId, { status: 'redo_fakturera' });
      ...
      if (activeTab === 'invoices') {
        ...
        const { data: invoicesData, error: invoicesError } = await getInvoices(organisationId, filters);
      ...
      if (activeTab === 'credit_notes') {
        ...
        const { data: creditNotesData, error: creditNotesError } = await getCreditNotes(organisationId, {});
  ...
  const handleManualSigning = async (invoiceId: string, file: File) => {
    try {
      ...
      setInvoices((prev: InvoiceWithRelations[]) => prev.map(inv =>
        inv.id === invoiceId
          ? { ...inv, signed_document_url: url, status: 'sent' }
          : inv
      ));
      ...
      success('Framgång', 'Signerad faktura uppladdad och sparad!');
    } catch (err) {
  ...
  const handleMarkAsPaid = async (invoiceId: string) => {
    ...
    if (result.error) {
      ...
      setInvoices((prevInvoices: InvoiceWithRelations[]) =>
        prevInvoices.map((inv: InvoiceWithRelations) =>
          inv.id === invoiceId ? { ...inv, status: 'paid' } : inv
        )
      );
      success('Framgång', `Faktura ${result.data.invoice_number} har markerats som betald.`);
    }
  };
  ...
  const handleSaveLineItem = async (itemToSave: { description: string; unit_price: number }) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        const savedItemsResult = await getSavedLineItems(organisationId!);
