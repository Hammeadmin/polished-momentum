                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    <div className="flex justify-end space-x-4 pt-4">
                        ...
                        <button type="submit" disabled={loading} className="inline-flex items-center px-5 py-2.5 border border-transparent rounded-lg shadow-sm text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 transition-all">
                            {loading ? <Loader2 className="w-4 h-4 animate-spin text-white" /> : (leadToEdit ? 'Spara Ändringar' : 'Skapa Lead')}
                        </button>
                    </div>
                </form>
            </div>
...
const LeadManagement: React.FC = () => {
    ...
    const loadLeads = async () => {
        ...
        const { data, error } = await getLeads(organisationId!, filters);
    ...
    return (
        <>
            <LeadFormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={loadLeads} leadToEdit={leadToEdit} organisationId={organisationId} />
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="flex items-center">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">{leads.length} aktiva leads</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-6 h-[calc(100vh-200px)]">
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 lg:col-span-1 xl:col-span-1 flex flex-col">
                        ...
                        {loading.leads ? <Loader2 className="w-6 h-6 mx-auto animate-spin text-blue-600" /> : (
