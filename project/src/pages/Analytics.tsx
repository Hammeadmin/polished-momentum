...
function Analytics() {
  ...
  const loadAnalyticsData = async () => {
    ...
    try {
      ...
      const salesTrends = salesData.map((item: any) => ({
        månad: item.month,
      ...
      // Transform lead data
      const leadAnalytics = leadData.map((item: any) => ({
      ...
      const customerMetrics = jobData.map((item: any) => ({
      ...
      const financialData = salesTrends.slice(-6).map((item: any) => ({
        månad: item.månad,
